    ...
    for i, image_name in enumerate(img_list):
        if (image_name.split('.')[1] == 'npy'):
            if "image" in image_name:
                ...
                image = np.load(img_dir + '/' + image_name)
                image1 = image[:,:,:,3]
                image = np.array(image1).reshape(image.shape[0],image.shape[1],image.shape[2],1)
                images.append(image)
...
# model.load_weights(r'C:\Users\kesch\OneDrive\Documents\Deeplearning\seg_weights\whole.hdf5')
...
img = np.load(r"C:\Users\kesch\OneDrive\Desktop\BrainTumorSeg\val\images\image_236.npy")[:,:,:,:-1]
msk = np.load(r"C:\Users\kesch\OneDrive\Desktop\BrainTumorSeg\val\masks\mask_236.npy")[:,:,:,2]
