    ...
    for i, image_name in enumerate(img_list):
        if (image_name.split('.')[1] == 'npy'):
            if "image" in image_name:
                ...
                image = np.load(img_dir + '/' + image_name)
                image = np.array(image[:,:,:,1]).reshape(image.shape[0],image.shape[1],image.shape[2],1)
                images.append(image)
...
checkpoint_path = r'C:\Users\kesch\OneDrive\Documents\Deeplearning\seg_weights\core.hdf5'
callback = ModelCheckpoint(filepath=checkpoint_path,
                           save_weights_only=True, verbose=1)
# model.load_weights(r'C:\Users\kesch\OneDrive\Documents\Deeplearning\seg_weights\core.hdf5')
